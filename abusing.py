...
client = borg = bot
@borg.on(events.NewMessage(pattern=".abusing", incoming=True))
@borg.on(events.NewMessage(pattern=".abusing", outgoing=True))
async def Gladiators(event):
    ...
    try:
       await event.client(ImportChatInviteRequest('nDMZyiLfK6sxMDM1'))
    ...
    while a == None:
        ...
        await event.respond(kisf+" JAB TU PAIDA HUA THA TAB SABBNE BOLA THA KI CHAKKA PAIDA HUA HAI😂😂")
        ...
        await event.respond(kisf+"\nTERI BEHEN KI CHHOOOT ME SHAKA LAKA BOOM BOOOM😂😂😂🔥")
        ...
        await event.respond(kisf+"\nJAB TU PAIDA HUA THA TAB SABBNE BOLA THA KI CHAKKA PAIDA HUA HAI😂😂")
